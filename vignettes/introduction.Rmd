...
try(fido$age <- "five") # No error, type checking is disabled
print(fido$age)
```
...
### Using with Various R Types

The `interface` package supports a wide range of R types:
...
person <- implement(ComplexPerson,
    ...
    factors = factor(c("A", "B", "A", "C")),
    dates = as.Date(c("2023-01-01", "2023-06-30")),
    times = as.POSIXct(c("2023-01-01 09:00:00", "2023-01-01 17:00:00")),
    metadata = list(last_updated = Sys.time())
)

print(person$hobbies)
print(person$scores)
print(person$factors)
print(person$dates)
print(person$times)
```

This example demonstrates that our interface system now supports various R types, including data.frames, data.tables, factors, dates, and times.
